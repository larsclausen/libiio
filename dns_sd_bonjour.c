	...
	void* 					info)
{
	...
	const char *hostname = CFStringGetCStringPtr(targetHost, encodingMethod);

	CFStringRef svcName = CFNetServiceGetName(netService);
	if (targetHost == NULL) {
		DEBUG("DNS SD: No valid name for service.\n");
		goto exit;
	}
	const char *name = CFStringGetCStringPtr(svcName, encodingMethod);
...
int dnssd_find_hosts(struct dns_sd_discovery_data ** ddata)
{
	...
	if (result == false) {
		...
		DEBUG("DSN SD: CFRunLoopRunInMode completed with %d\n", runRes);
	...
	DEBUG("DNS SD: Completed service discovery: %d.\n", ret);
